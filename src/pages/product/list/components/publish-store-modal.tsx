...
function PublishStoreModal({
  ...
  const [channelStatusDraft, setChannelStatusDraft] =
    useState<ProductStoreChannelStatus>('on');
  ...
  function handleSellStatusChange(value: string) {
    const nextValue = value as ProductStoreSellStatus;
    setSellStatusDraft(nextValue);

    if (nextValue === 'unsellable') {
      setChannelStatusDraft('off');
      return;
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await onSubmit({
        ...
        channelStatus:
          sellStatusDraft === 'unsellable' ? 'off' : channelStatusDraft,
      ...
      <div className={styles.modalContent}>
        <div className={styles.summaryCard}>
          <Typography.Text className={styles.summaryText}>
            当前将对已勾选的 {productCount} 个商品批量设置店铺可售与上下架状态。
          </Typography.Text>
        </div>
        ...
        <div className={styles.optionPanel}>
          ...
          <div className={styles.optionRow}>
            <div className={styles.optionLabel}>设置上下架：</div>
            <Radio.Group
              disabled={sellStatusDraft === 'unsellable'}
              value={channelStatusDraft}
              onChange={(value) => setChannelStatusDraft(value as ProductStoreChannelStatus)}
            >
